...
def test_ethics_metrics():
    ...
    try:
        ...
        # Test bias ratio
        group_outcomes = {
            "group_a": [0.8, 0.85, 0.9],
            "group_b": [0.6, 0.65, 0.7],
...
def test_guards():
    ...
    try:
        from penin.omega.guards import (
            sigma_guard, ir_to_ic_contractive, SigmaGuardPolicy,
        ...
        all_passed, results = combined_guard_check(
            good_metrics, contracting_risk, policy
        )
...
def test_scoring():
    ...
    try:
        from penin.omega.scoring import (
            normalize_series, ema, linf_harmonic, score_gate,
            ScoreVerdict, harmonic_mean, aggregate_scores,
            compute_delta_linf, ScoreTracker
        )
        ...
        # Test canary zone
        verdict, score, details = score_gate(
            U=0.6, S=0.6, C=0.4, L=0.5, tau=0.7, canary_margin=0.1
        )
        print(f"  ✓ Canary zone: {verdict.value}, score={score:.4f}")
        
        # Test fail zone
        verdict, score, details = score_gate(
            U=0.3, S=0.3, C=0.8, L=0.2, tau=0.7
        )
...
def test_caos():
    ...
    try:
        from penin.omega.caos import (
            compute_caos_plus, apply_saturation, compute_caos_harmony,
            caos_gradient, CAOSConfig, CAOSTracker
        )
...
def test_sr():
    ...
    try:
        from penin.omega.sr import (
            compute_sr_omega, harmonic_mean, geometric_mean, min_soft_pnorm,
            compute_awareness_score, compute_autocorrection_score,
            compute_metacognition_score, SRConfig, SRTracker
        )
        ...
        # Test with ethics veto
        sr_veto, details = compute_sr_omega(
            awareness, False, autocorrection, metacognition
        )
        ...
        sr_h, _ = compute_sr_omega(
            awareness, ethics_ok, autocorrection, metacognition, config_harmonic
        )
        
        config_minsoft = SRConfig(aggregation="min_soft", p_norm=-5.0)
        sr_m, _ = compute_sr_omega(
            awareness, ethics_ok, autocorrection, metacognition, config_minsoft
        )
        
        config_geometric = SRConfig(aggregation="geometric")
        sr_g, _ = compute_sr_omega(
            awareness, ethics_ok, autocorrection, metacognition, config_geometric
        )
