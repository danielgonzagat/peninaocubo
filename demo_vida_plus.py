...
class LifePlusSystem:
    ...
    def run_cycle(self) -> dict:
        ...
        print(
            f"🔥 KRATOS: {'🚀 EXPLORE' if kratos_result['exploration_allowed'] else '🛡️ STANDARD'} (φ={kratos_result['phi']:.3f})"
        )
        ...
        self.swarm.emit_heartbeat(
            {"phi": metrics["phi"], "sr": metrics["sr"], "g": metrics["G"], "health": 1.0 if verdict.ok else 0.5}
        )
        ...
        return {"cycle": self.cycle, "verdict": verdict.ok, "alpha_eff": verdict.alpha_eff, "metrics": metrics}
    ...
    def _run_market(self):
        ...
        self.market.submit_need(Need(agent=f"agent_{self.cycle}", resource="cpu_time", qty=10.0, max_price=2.0))

        self.market.submit_offer(Offer(agent="provider", resource="cpu_time", qty=20.0, price=1.5))
    ...
    def _record_decision(self, verdict: LifeVerdict, metrics: dict):
        ...
        if verdict.ok:
            self.checkpoint.checkpoint(state={"cycle": self.cycle, "metrics": metrics}, force=True, verdict="allowed")
    ...
    def _evolve(self, metrics: dict):
        ...
        # Evaluate fitness
        fitness = self.darwin.evaluate_variant(variant, life_ok=True, metrics=metrics)
