...
class ProviderStats:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {**asdict(self), "health": self.health.value, "success_rate": self.success_rate()}
...
class CircuitBreaker:
    ...
    def __init__(self, failure_threshold: int = 5, recovery_timeout_s: float = 60.0, half_open_max_calls: int = 1):
        """
        Args:
...
class EnhancedMultiLLMRouter:
    ...
    def __init__(
        ...
        # Budget tracking
        self.budget_tracker = BudgetTracker(daily_budget_usd=daily_budget_usd or settings.PENIN_BUDGET_DAILY_USD)
    ...
    def _save_state(self):
        ...
        try:
            state = {
                ...
                "provider_stats": {pid: stats.to_dict() for pid, stats in self.provider_stats.items()},
    ...
    async def _call_provider_with_tracking(
        self, provider: BaseProvider, messages: List[Dict[str, Any]], **kwargs
    ) -> LLMResponse:
    ...
    async def ask(
        ...
        available_providers = [
            p for p in self.providers if self.provider_stats[p.provider_id].health != ProviderHealth.CIRCUIT_OPEN
        ]
        ...
        tasks = [
            self._call_provider_with_tracking(p, messages, tools=tools, system=system, temperature=temperature)
        ...
        valid_responses = [(r, self.provider_stats[r.provider_id]) for r in results if isinstance(r, LLMResponse)]
        ...
        scored_responses = [(r, stats, self._score_response(r, stats)) for r, stats in valid_responses]
        best_response, best_stats, best_score = max(scored_responses, key=lambda x: x[2])

        # Update budget tracker
        self.budget_tracker.add_usage(
            best_response.cost_usd, best_response.prompt_tokens + best_response.completion_tokens
        )
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_breakers": {pid: breaker.get_state().value for pid, breaker in self.circuit_breakers.items()}
            if self.enable_circuit_breaker
            else {},
            "config": {
...
def create_enhanced_router(
    providers: List[BaseProvider], daily_budget_usd: float = 5.0, **kwargs
