...
def _extract_metrics(*args, **kwargs) -> Dict[str, Any]:
    ...
    for k in ("ece","rho_bias","consent","eco_ok","risk_rho","caos_phi","C","A","O","S","sr","L_inf","dL_inf","G","base_alpha"):
        if k in kwargs:
            out[k] = kwargs[k]
...
def _wrap_promote(fn, where: str):
    ...
    def wrapper(*args, **kwargs):
        ...
        verdict = life_equation(**{k:v for k,v in metrics.items() if k in inspect.signature(life_equation).parameters})
        if not verdict.ok:
