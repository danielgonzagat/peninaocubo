...
class MistralTuner:
    ...
    def prepare_training_data(
        ...
        output_path: str = "/tmp/mistral_tuning.jsonl"
    ) -> str:
        """
    ...
    def query_tuned_model(self, model_id: str, current_state: Dict) -> Dict[str, float]:
        ...
        return {
            k: v * 1.01 for k, v in current_state.items()
        }
...
class OpenAITuner:
    ...
    def prepare_sft_data(
        self,
        cycles: List[Dict[str, Any]],
        output_path: str = "/tmp/openai_sft.jsonl"
    ) -> str:
    ...
    def prepare_dpo_data(
        self,
        cycles: List[Dict[str, Any]],
        output_path: str = "/tmp/openai_dpo.jsonl"
    ) -> str:
        ...
        for i in range(len(sorted_cycles) - 1):
            ...
            item = {
                ...
                "preferred_output": [
                    {"role": "assistant", "content": self._format_optimized_params(
...
class AutoTuner:
    ...
    def update(self, metrics: Dict[str, float]) -> Dict[str, float]:
        ...
        for param_name in ["kappa", "lambda_c", "w_U", "w_S", "w_C", "w_L", "beta_min"]:
            ...
            # Clip delta
            delta = max(-self.config.max_delta_per_cycle, 
                       min(self.config.max_delta_per_cycle, delta))
    ...
    def save_state(self, path: str = "/tmp/tuner_state.json") -> None:
        ...
        state = {
            ...
            "adagrad_state": {
                "accumulated_gradients": self.state.accumulated_gradients
            }
        }
