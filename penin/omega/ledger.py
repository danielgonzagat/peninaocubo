...
class WORMLedger:
    ...
    def append_record(self, record: RunRecord, artifacts: Optional[Dict[str, Any]] = None) -> str:
        """
...
# -----------------------------------------------------------------------------
# Backwards-compatible API for older tests (P0 suites)
# -----------------------------------------------------------------------------
...
class SQLiteWORMLedger:
    """Minimal SQLite WORM ledger compatible with legacy tests.
    ...
    Provides:
      ...
      - verify_chain()
      - close()
    Ensures WAL mode and busy_timeout are set for concurrency checks.
    """
    ...
    def _get_last_hash(self) -> str:
        ...
        cur.execute("SELECT hash FROM events ORDER BY id DESC LIMIT 1")
        row = cur.fetchone()
        return row[0] if row else "genesis"
    ...
    def append(self, event: WORMEvent) -> str:
        ts = time.time()
        # Serialize writes with a simple retry loop to avoid sqlite busy errors
        with self._lock:
            for _ in range(5):
                try:
                    cur = self._conn.cursor()
                    # Compute prev from current tail under the same lock to avoid races
                    prev = self._tail
                    payload = {
                    ...
                    cur.execute(
                        "INSERT INTO events (type, cycle_id, data, ts, prev, hash) VALUES (?,?,?,?,?,?)",
                        (
                            event.event_type,
                            event.cycle_id,
    ...
    def query(self, limit: int = 100) -> List[Dict[str, Any]]:
        cur = self._conn.cursor()
        cur.execute("SELECT type, cycle_id, data, ts, prev, hash FROM events ORDER BY id DESC LIMIT ?", (limit,))
    ...
    def verify_chain(self) -> tuple[bool, Optional[str]]:
        cur = self._conn.cursor()
        cur.execute("SELECT type, cycle_id, data, ts, prev, hash FROM events ORDER BY id")
        prev = "genesis"
        for idx, (etype, cycle_id, data, ts, stored_prev, stored_hash) in enumerate(cur.fetchall(), 1):
            if stored_prev != prev:
                return False, f"Chain break at row {idx}"
            payload = {
                "type": etype,
                "cycle_id": cycle_id,
            ...
            if calc != stored_hash:
                return False, f"Hash mismatch at row {idx}"
            prev = stored_hash
        return True, None
...
class JSONLWORMLedger:
    """Very small JSONL-based ledger stub for import compatibility."""
