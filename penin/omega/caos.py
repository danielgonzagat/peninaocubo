...
def compute_caos_plus(C: float, 
                      A: float,
                      O: float,
                      S: float,
                      kappa: float,
                      config: Optional[CAOSConfig] = None) -> Tuple[float, Dict]:
    """
...
def compute_caos_harmony(C: float,
                        A: float,
                        O: float,
                        S: float,
...
def caos_gradient(C: float,
                  A: float,
                  O: float,
                  S: float,
                  kappa: float,
                  delta: float = 0.001) -> Dict[str, float]:
    """
...
class CAOSTracker:
    ...
    def update(self, C: float, A: float, O: float, S: float, kappa: float = 2.0):
        ...
        self.history.append({
            "C": C, "A": A, "O": O, "S": S,
            "C_ema": self.C_ema, "A_ema": self.A_ema,
            "O_ema": self.O_ema, "S_ema": self.S_ema,
            "caos": caos_value, "caos_ema": self.caos_ema,
            "details": details
        })
