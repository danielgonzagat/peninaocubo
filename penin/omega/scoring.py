...
def ema(values: list[float], alpha: float = 0.3, init_value: float | None = None) -> list[float]:
    """
...
def harmonic_mean(
    ...
    denominator = sum(w / v for w, v in zip(weights, values, strict=False))
...
def score_gate(
    ...
    details = {
        "components": {"U": U, "S": S, "C": C, "L": L},
        "weights": {"wU": wU, "wS": wS, "wC": wC, "wL": wL},
        "contributions": {
            "U_contrib": wU * U,
            "S_contrib": wS * S,
            "C_contrib": -wC * C,
