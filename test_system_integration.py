...
def test_router_with_observability():
    ...
    print(f"✓ Budget tracking: ${stats['daily_spend_usd']:.4f} spent, "
          f"{stats['budget_used_pct']:.1f}% used")
...
def test_ledger_operations():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i in range(10):
            ...
            conn.execute(
                "INSERT INTO events (timestamp, event_type, cycle_id, data, hash, prev_hash) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (
