...
def demo_life_equation():
    ...
    verdict = life_equation(
        base_alpha=1e-3,
        ethics_input={"ece": 0.005, "rho_bias": 1.01, "fairness": 0.9, "consent_valid": True, "eco_impact": 0.3},
    ...
    print(
        f"  ✓ Métricas: phi={verdict.metrics.get('phi', 0):.3f}, "
...
def demo_market():
    ...
    needs = [Need("agent1", "cpu", 100.0, 5.0), Need("agent2", "memory", 50.0, 3.0), Need("agent3", "cpu", 50.0, 4.5)]

    offers = [Offer("agent4", "cpu", 120.0, 4.0), Offer("agent5", "memory", 80.0, 2.5)]
    ...
    for i, (need, offer, qty) in enumerate(trades):
        print(
            f"    Trade {i + 1}: {need.agent} compra {qty:.1f} {need.resource} de {offer.agent} por ${offer.price:.2f}"
        )
...
def demo_checkpoint():
    ...
    state = {"cycle": 42, "phi": 0.78, "sr": 0.87, "G": 0.90, "alpha_eff": 0.0008}
