...
def test_imports() -> Dict[str, bool]:
    ...
    try:
        from penin.equations import (
            penin_update, PeninState,
            compute_linf_meta, LInfConfig,
            compute_caos_plus_complete, CAOSConfig,
            compute_sr_omega_infinity, SRConfig,
            death_gate, DeathConfig,
            ir_to_ic, ContractivityConfig,
            expected_possession_value, EPVConfig,
            compute_agape_index, AgapeConfig,
            omega_sea_coherence, OmegaSEAConfig,
            auto_tune_hyperparams, AutoTuningConfig,
            lyapunov_check, LyapunovConfig,
            organizational_closure_index, OCIConfig,
            delta_linf_compound_growth, DeltaLInfConfig,
            anabolize_penin, AnabolizationConfig,
            sigma_guard_check, SigmaGuardConfig,
        )
    ...
    try:
        from penin.omega.caos import phi_caos, compute_caos_plus
        from penin.omega.sr import compute_sr_omega
        from penin.guard.sigma_guard_service import app
...
def test_basic_functionality() -> bool:
    ...
    try:
        import numpy as np
        from penin.equations import (
            PeninState, Evidence, ControlPolicy, ProjectionConstraints,
            penin_update, compute_linf_meta,
            Metric, CostComponents, EthicalGates,
            sigma_guard_check
        )
...
def test_ethical_fail_closed() -> bool:
    ...
    try:
        from penin.equations import (
            sigma_guard_check, compute_linf_meta,
            Metric, CostComponents, EthicalGates, LInfConfig
        )
...
def test_worm_ledger() -> bool:
    ...
    try:
        ...
        if root:
            print(f"✅ WORM ledger funcionando!")
            print(f"   - Merkle root: {root[:16]}...")
            print(f"   - 3 eventos registrados")
            
            # Verificar imutabilidade (eventos gravados)
            with open(test_ledger, 'r') as f:
                lines = f.readlines()
                if len(lines) == 3:
                    print(f"✅ Imutabilidade verificada: 3 linhas gravadas")
                else:
...
def test_mathematical_guarantees() -> bool:
    ...
    try:
        import numpy as np
        from penin.equations import (
            lyapunov_check, ir_to_ic,
            harmonic_mean_weighted, Metric
        )
        ...
        if is_contractive and V_next < V_curr:
            ...
            print(f"❌ Lyapunov FALHOU: deveria ser contrativo")
            return False
