...
def test_imports():
    ...
    try:
        ...
        from penin.omega import (
            scoring, caos, ethics_metrics, guards, sr,
            tuner, acfa, ledger, mutators, evaluators, runners
        )
...
def test_scoring_module():
    ...
    try:
        from penin.omega.scoring import (
            harmonic_mean, linf_harmonic, score_gate, 
            normalize_series, ema, ScoreGateVerdict
        )
        ...
        verdict, score = score_gate(
            ...
            wU=0.25, wS=0.25, wC=0.25, wL=0.25,
            tau=0.7
        )
...
def test_caos_module():
    ...
    try:
        from penin.omega.caos import (
            phi_caos, CAOSComponents, CAOSPlusEngine
        )
...
def test_ethics_module():
    ...
    try:
        from penin.omega.ethics_metrics import (
            EthicsCalculator, EthicsGate, EthicsMetrics
        )
        ...
        rho_bias, bias_evidence = calc.calculate_bias_ratio(
            predictions, targets, protected_attrs
        )
        assert rho_bias >= 1.0, f"Bias ratio invalid: {rho_bias}"
        print(f"  ✅ Bias ratio: {rho_bias:.4f}")
        
        # Test fairness
        fairness, fairness_evidence = calc.calculate_fairness(
            predictions, targets, protected_attrs
        )
...
def test_evaluators_module():
    ...
    try:
        from penin.omega.evaluators import (
            ComprehensiveEvaluator, UtilityEvaluator,
            StabilityEvaluator, CostEvaluator, LearningEvaluator
        )
