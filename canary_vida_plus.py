...
def run_integrated_canary(cycles: int = 5) -> dict:
    ...
    for cycle in range(cycles):
        ...
        darwin_score = darwinian_score(
            verdict.ok,
            phi_kratos,
            verdict.metrics.get("sr", 0),
            G,
            verdict.metrics.get("L_inf", 0)
        )
    ...
    criteria_met = {
        ...
        "consciousness_below_threshold": all(c.get("consciousness_spi", 0.1) < 0.1 for c in canary_history)  # Critério mais específico
    }
