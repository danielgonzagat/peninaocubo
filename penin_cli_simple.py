...
def cmd_evolve(args):
    ...
    try:
        if args.dry_run:
            ...
            champion_config = {"temperature": 0.7, "top_p": 0.9, "max_tokens": 1000}

            generator = ChallengerGenerator(seed=42)
            challengers = generator.generate_from_champion(champion_config, args.n_challengers)
            ...
            # Ciclo completo
            result = quick_evolution_cycle(n_challengers=args.n_challengers, budget_usd=args.budget, seed=42)
...
def cmd_promote(args):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ledger = WORMLedger(db_path=Path(tmpdir) / "promote.db", runs_dir=Path(tmpdir) / "runs")
