...
from penin.omega.ledger import WORMEvent, SQLiteWORMLedger, JSONLWORMLedger, WORMLedger
...
def test_consent_valid():
    ...
    metadata = {"id": "dataset_123", "user_consent": True, "privacy_policy_accepted": True}
...
def test_consent_invalid():
    ...
    metadata = {"id": "dataset_456", "user_consent": False}
...
def test_create_ethics_attestation():
    ...
    cycle_id = "cycle_001"
    seed = 42
    dataset = {"id": "test_dataset", "user_consent": True, "privacy_policy_accepted": True}
...
def test_sqlite_worm_concurrent_writes():
    ...
    def write_events(thread_id: int, count: int):
        try:
            for i in range(count):
                event = WORMEvent(
                    event_type="TEST", cycle_id=f"thread_{thread_id}_event_{i}", data={"thread": thread_id, "index": i}
                )
                ledger.append(event)
    ...
    threads = [threading.Thread(target=write_events, args=(i, 10)) for i in range(3)]
...
def test_worm_chain_integrity():
    ...
    # Adicionar 5 eventos
    for i in range(5):
        event = WORMEvent(event_type="CHAIN_TEST", cycle_id=f"cycle_{i}", data={"index": i})
        ledger.append(event)
...
async def test_router_budget_enforcement():
    ...
    class MockProvider:
        async def chat(self, messages, **kwargs):
            return LLMResponse(content="test response", latency_s=0.1, provider="mock", cost_usd=0.6, total_tokens=1000)
