...
def demo_scoring_system():
    ...
    # Scorer U/S/C/L
    uscl_scorer = USCLScorer(
        weights={"wU": 0.3, "wS": 0.3, "wC": 0.2, "wL": 0.2},
        tau=0.7
    )
    ...
    linf_scorer = LInfinityScorer(
        metric_weights={
            "rsi": 0.2, "synergy": 0.2, "novelty": 0.2,
    ...
    linf_metrics = {
        ...
        "stability": 0.9, "viability": 0.8, "cost": 0.3
    }
...
async def demo_router_with_budget():
    ...
    try:
        ...
        for i in range(3):
            ...
            status = router.get_budget_status()
            print(f"   Chamada {i+1}: {response.provider} (${response.cost_usd:.4f}) - "
