...
def demo_modules_integration():
    ...
    ethics_result = calculate_and_validate_ethics(
        {"consent": True, "eco": True, "rho": 0.7}, 
        {"ethics": {"ece_max": 0.01}}, 
        seed=42
    )
    print(f"✅ Ethics: ECE={ethics_result['metrics']['ece']:.4f}, "
          f"evidência={ethics_result['evidence_hash']}")
    ...
    def mock_model(prompt): return '{"extracted": "data"}' if "json" in prompt else "response"
    ...
    print(f"✅ ACFA: champion={league_status['champion']['run_id'][:8] if league_status['champion']['run_id'] else 'None'}...")
...
def demo_evolution_cycle():
    ...
    if result.tuning_result:
        ...
        if tuning.get("tuning_active"):
            updates = len([u for u in tuning.get("parameter_updates", {}).values() 
                          if "error" not in u])
